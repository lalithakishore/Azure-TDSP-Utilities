...
yaml_file <- choose.files(caption = "Please select a YAML file", multi = FALSE)
...
if(((config$Target %in% config$CategoricalColumns) & length(config$CategoricalColumns) > 1) | (!(config$Target %in% config$CategoricalColumns) & length(config$CategoricalColumns)>=1))
{
  ...
  if (config$Target %in% config$CategoricalColumns){
    x22 = data[,config$CategoricalColumns[config$CategoricalColumns != config$Target]]
  } else{
    x22 = data[,config$CategoricalColumns]
  }
